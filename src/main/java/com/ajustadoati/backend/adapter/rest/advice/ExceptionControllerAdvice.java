package com.ajustadoati.backend.adapter.rest.advice;

import com.ajustadoati.backend.adapter.rest.exception.ErrorResponse;
...
public class ExceptionControllerAdvice {
  ...
  public HttpEntity<ErrorResponse> handleMassActionNotFoundException(
     ...
     * Handles a case when entity is not found
     *
     * @param ex      {@link UserNotFoundException}
     * @param request input request
     * @return {@link HttpEntity} containing standard body in case of errors.
     */
    @ExceptionHandler(UserNotFoundException.class)
    public HttpEntity<ErrorResponse> handleUserNotFoundException(
        UserNotFoundException ex, HttpServletRequest request) {

        String message = ex.getCause() != null ? ex.getCause().getMessage() : ex.getMessage();

        return handle(ex, request, HttpStatus.NOT_FOUND, NOT_FOUND_TITLE, message);
    }
